...
from guitarplay.modelpy.hands import base_R
...
_FOREARM_DOFS: Dict[str, Dof] = {
    "forearm_tx": Dof(
        ...
        # initialization, see `robopianist/suite/tasks/base_R.py`.
        joint_range=(-1, 1),
    ),
    "forearm_ty": Dof(
    ...
    "forearm_tz": Dof(
        joint_type="slide", axis=(0, 1, 0), stiffness=1000, joint_range=(-0.07, 0.07)
    ),
    "forearm_roll": Dof(
...
_THUMBTIP_OFFSET = 0.030
_THUMBTIP_XOFFSET=0.004

# Which dofs to add to the forearm.
_DEFAULT_FOREARM_DOFS = ("forearm_tx", "forearm_ty","forearm_tz")


class ShadowHand(base_R.Hand):
    """A Shadow Hand E3M5."""

    def _build(
        self,
        name: Optional[str] = None,
        side: base_R.HandSide = base_R.HandSide.RIGHT,
        ...
        """
        if side == base_R.HandSide.RIGHT:
            self._prefix = "rh_"
            xml_file = consts.RIGHT_SHADOW_HAND_XML
        elif side == base_R.HandSide.LEFT:
            self._prefix = "lh_"
    ...
    def _parse_mjcf_elements(self) -> None:#减少动作空间与动作范围
        ...
        # Remove "grasp_site".
        if self._hand_side == base_R.HandSide.RIGHT:
    ...
    def _add_mjcf_elements(self) -> None:
        ...
        for tip_name in consts.FINGERTIP_BODIES:
            ...
            tip_site = tip_elem.add(
                "site",
                name=tip_name + "_site",
                pos=(0.0+xoffset, 0.0, offset+0.006),
                type="sphere",
                size=(0.001,),
    ...
    @property
    def hand_side(self) -> base_R.HandSide:
...
class ShadowHandObservables(base_R.HandObservables):
