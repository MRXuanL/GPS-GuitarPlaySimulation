...
from guitarplay.modelpy.hands import base
from guitarplay.modelpy.hands import shadow_hand_constants as consts

INITIAL_NAME=['lh_WRJ2','lh_WRJ1','lh_THJ5','lh_THJ4','lh_THJ3',
              'lh_THJ2','lh_THJ1','lh_FFJ4','lh_FFJ3','lh_FFJ0',
              'lh_MFJ4','lh_MFJ3','lh_MFJ0','lh_RFJ4','lh_RFJ3',
              'lh_RFJ0','lh_LFJ5','lh_LFJ4','lh_LFJ3','lh_LFJ0']

INITIAL_POS=[0,0,-0.429,1.22,0.105,0.147,0.664,-0.0419,1.2,0,0,1.2,0,0,1.2,0,0,0,0.948,0]
INITIAL_RANGE=[
    (-0.0001,0.0001),
    (0,0.789),
    (-0.0001,0.0001),
...
_FOREARM_DOFS: Dict[str, Dof] = {
    "forearm_tx": Dof(
        ...
        axis=(-1, 0, 0),
        stiffness=300,
        joint_range=(-0.005, 0.005),
    ),
    "forearm_ty": Dof(
        joint_type="slide", axis=(0, 0, 1), stiffness=300, joint_range=(-0.01, 0.10)
    ),
    "forearm_tz": Dof(
        joint_type="slide", axis=(0, 1, 0), stiffness=1000, joint_range=(-0.1, 0.02)
    ),
    "forearm_roll": Dof(
...
_THUMBTIP_OFFSET = 0.020
_THUMBTIP_XOFFSET=0.0
_THUMBTIP_YOFFSET=-0.007
...
class ShadowHand(base.Hand):
    ...
    def _parse_mjcf_elements(self) -> None:#减少动作空间与动作范围
        # change left hand ctrlrange
        for i,name in enumerate(INITIAL_NAME):
            if name[-1]!='0':
                joint= mjcf_utils.safe_find(self._mjcf_root, "joint",name)
                joint.range=INITIAL_RANGE[i]
            aname=name[0:3]+'A_'+name[3:]
            actuator =mjcf_utils.safe_find(self._mjcf_root, "actuator",aname)
            actuator.ctrlrange =INITIAL_RANGE[i]
    ...
    def _add_mjcf_elements(self) -> None:
        ...
        for tip_name in consts.FINGERTIP_BODIES:
            ...
            xoffset= _THUMBTIP_XOFFSET if tip_name == "thdistal" else 0
            yoffset= _THUMBTIP_YOFFSET if tip_name == "thdistal" else 0
            tip_site = tip_elem.add(
                "site",
                name=tip_name + "_site",
                pos=(0.0+xoffset, yoffset, offset+0.006),
                type="sphere",
                size=(0.001,),
